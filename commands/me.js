...
import MeHelp from '../components/me/MeHelp';
import IPOnlyError from '../components/me/IPOnlyError';
...
const me = ({meHelp, ipOnly, modules, development, format = 'Newline'}) => {
	...
	React.useEffect(() => {
		if (APIKey.length > 0){
			setLoading(true);
			setIsError(false);
			sendRequestToGRE('GeoIP', serialize({
				key: APIKey,
		...
		:(isKeyChecked)?
			...
			:
				<>
					...
					{
						loading?
							<Text><Spinner type="earth" />Fetching the {(ipOnly)?'IP Address':'data'}...</Text>
						:
							(ipOnly)?
								<><Text><Text color="#3FC25A">⨠</Text> Your IP Address: {data.data.ip}</Text><Text> </Text></>
							:
								(format === 'Newline')?
									<>
										<Text> </Text>
...
me.propTypes = {
	meHelp: PropTypes.bool,
	ipOnly: PropTypes.bool,
